...
void add_sphere( struct matrix * edges,
  ...
  longStop = num_steps;
	int p0,p1,p2,p3;
	int p0x,p0y,p0z,p1x,p1y,p1z,p2x,p2y,p2z,p3x,p3y,p3z;
	int numPoints = points->lastcol;
 	num_steps++;
 for ( lat = latStart; lat < latStop; lat++ ) {
	 for ( longt = longStart; longt <= longStop; longt++ ) {
		 ...
		 p0y = points->m[1][p0];
		 p0z = points->m[2][p0];
		 p1=(p0+1)%numPoints; //use modulo so we can connect start to end
		 p1x = points->m[0][p1];
		 p1y = points->m[1][p1];
		 p1z = points->m[2][p1];
		 p2=(p0+num_steps)%numPoints;
		 p2x = points->m[0][p2];
		 p2y = points->m[1][p2];
		 p2z = points->m[2][p2];
		 p3=(p0+1+num_steps)%numPoints;
		 p3x = points->m[0][p3];
		 p3y = points->m[1][p3];
		 p3z = points->m[2][p3];
		 add_polygon(edges,p1x,p1y,p1z,p2x,p2y,p2z,p3x,p3y,p3z);
